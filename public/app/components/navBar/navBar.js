class NavBar extends HTMLElement {
    constructor() {
        super();
        this.attachShadow({ mode: "open" });
    }
    connectedCallback() {
        this.render();
    }
    render() {
        if (this.shadowRoot) {
            this.shadowRoot.innerHTML = `
            <link rel="stylesheet" href="./app/components/navBar/navBar.css">
            <section>
                <section>
                    <img src="data:image/png;base64,">
                    <img src="https://cdn-icons-png.flaticon.com/512/54/54481.png">
                    <input type="text" placeholder="Search games, hardware, news, etc">
                    <p>All categories</p>
                </section>
                <section>
                    <button>
                        <img src="https://cdn-icons-png.flaticon.com/512/3246/3246789.png">
                        <h3>Support</h3>
                    </button>
                    <button>
                        <img src="https://cdn-icons-png.flaticon.com/512/4225/4225964.png">
                        <h3>Wish List</h3>
                    </button>
                    <button>
                        <img src="https://cdn-icons-png.flaticon.com/512/2838/2838838.png">
                        <h3>Cart</h3>
                    </button>
                    <button>
                        <img src="https://cdn-icons-png.flaticon.com/512/456/456212.png">
                        <h3>Log in / Sign up</h3>
                    </button>
                </section>
            </section>
            
            <section>
                <button>
                    <img src="https://cdn-icons-png.flaticon.com/128/726/726569.png">
                    <h3>My Nintendo Store</h3>
                </button>
                <button>
                    <img src="https://cdn-icons-png.flaticon.com/512/7235/7235310.png">
                    <h3>Games</h3>
                </button>
                <button>
                    <img src="https://cdn-icons-png.flaticon.com/128/871/871510.png">
                    <h3>Nintendo Switch</h3>
                </button>
                <button>
                    <img src="https://cdn-icons-png.flaticon.com/128/21/21781.png">
                    <h3>News Y Events</h3>
                </button>
                <button>
                    <img src="https://cdn-icons-png.flaticon.com/512/2893/2893811.png">
                    <h3>Play Nintendo</h3>
                </button>
            </section>
            `;
        }
    }
}
customElements.define("my-bar", NavBar);
export default NavBar;
